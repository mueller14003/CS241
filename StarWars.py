        ...
        if self.ship.alive:
            if arcade.key.SPACE in self.held_keys:
                if self.shoot_timer == DEFAULT_SHOOT_TIMER and not self.mute:
                    arcade.play_sound(self.ship_blast)
        ...
        if self.player_two.alive:
            if arcade.key.GRAVE in self.held_keys:
                if self.shoot_timer_2 == DEFAULT_SHOOT_TIMER and not self.mute:
                    arcade.play_sound(self.p_2_blast)
    ...
    def shoot(self, ship):
        ...
        if not self.pause:
            ...
            if ship.type == 0:
                ...
                bullet = EnemyLaser()
                if not self.mute:
                    arcade.play_sound(self.enemy_blast)
                distance = 50
            elif ship.type == 2:
                bullet = LaserTwo()
            if (ship.type == 0 and self.power_up) or (ship.type == 2 and self.power_up_2):
                    bullet.image = "Files/PURPLE_LASER.png"
